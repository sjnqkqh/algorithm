...
public class 원재의_메모리_복구하기 {
	...
	public static void main(String args[]) throws Exception {
		...
		// 22시 00분 시작 맥주가 진짜로 얼기 전에 끝낸다.
		int count = Integer.parseInt(sc.nextLine());

		for (int i = 0; i < count; i++) {
			char[] bits = sc.nextLine().toCharArray();
			char[] repairing = new char[bits.length];
			int repairCount = 0;

			// 0으로 가득찬 상태 만들기
			for (int o = 0; o < repairing.length; o++) {
				repairing[o] = '0';
			}
			...
			for (int j = 0; j < bits.length; j++) {
				char origin = bits[j];
				char repair = repairing[j];

				// 원본과 다르면 끝까지 해당 비트로 채우기
				if (origin != repair) {
					repairCount++;
					for (int k = j; k < repairing.length; k++) {
						repairing[k] = origin;
					}
				}
			}

			System.out.println("#" + (i + 1) + " " + repairCount);
